...
namespace AirView.Domain.Core
{
        ...
        IAggregateRoot<TId>
    {
        ...
        void IAggregateRoot.ClearUncommitedEvents() => _uncommitedEvents.Clear();

        void IAggregateRoot.RaiseEvent<TEvent>(TEvent @event) => Raise(@event);

        IEnumerable<IDomainEvent> IAggregateRoot.UncommittedEvents => _uncommitedEvents;
